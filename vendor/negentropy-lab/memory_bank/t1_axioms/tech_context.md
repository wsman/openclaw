...
- 前端位于外部仓`opendoge-ui`；当前工作环境为 Windows 10，常用路径为`D:\Users\WSMAN\Desktop\OpenDoge\opendoge-ui`，其他环境需改成对应绝对路径。
...
#### 3.2.1 Legacy WebSocket协议（兼容入口）
- **入口**: `src/index.ts`（legacy入口）
- **启动**: `npm run dev:legacy` / `npm run start:legacy`
- **路径**: `/ws`
...
#### 3.2.2 主运行时（默认 npm 链路）
- **入口**: `server/index.ts` → `server/bootstrap/createNegentropyServer.ts`
- **启动**: `npm run dev` / `npm start`
- **职责**: API-only + Colyseus房间 + Agent/Authority/Cluster/Discovery 主运行态
- **默认装配**: authority 状态/监控路由、开发态 `/colyseus` 面板（开发环境且未禁用时）
- **边界**: 默认主链路不直接暴露 `/v1/chat/completions` / `/v1/responses`

#### 3.2.3 Gateway WebSocket（独立运行时）
- **入口**: `server/gateway/index.ts` → `server/gateway/server.impl-with-ws.ts`
- **状态**: 独立模块门面/脚本调用路径，不是当前 npm 默认主入口
...
#### 3.2.4 HTTP REST API
- **端口**: Gateway服务端口
- **协议**: OpenAI风格HTTP端点（位于`server/gateway/`，存在简化路由与独立适配器两种实现）
- **认证**: 目标为Bearer Token/JWT；当前实现以简化路由与集成桥接为主
- **特性**: `server.impl(-with-ws).ts` 返回简化/mock风格响应；`openai-http.ts` 为独立适配器风格实现；是否启用取决于所选运行时

#### 3.2.5 多入口说明
- **默认开发入口**: `server/index.ts` → `npm run dev`
- **默认生产入口**: `dist/server/index.js` → `npm start`
- **Legacy入口**: `src/index.ts` → 简化WebSocket服务（自定义协议）
- **Gateway模块入口**: `server/gateway/index.ts` → 完整Gateway生态系统（HTTP + WebSocket RPC + 插件 + 监控），需由脚本/调用方显式装配
...
#### 3.3.2 双插件体系
- **Gateway运行时插件体系**: `server/gateway/plugins/`，提供 PluginManager / PluginRegistry / PluginValidator / 示例插件与 OpenAI proxy 插件路径
- **OpenClaw/authority兼容插件体系**: `server/plugins/`，提供 `PluginManager`、CLI、`PluginKind` 与生命周期语义
- **HotReloadWatcher**: 当前未形成统一默认热重载主链路；`server/gateway/plugins/` 中目录监听仍为待实现占位

### 3.4 监控系统技术栈
**宪法依据**: §504监控系统公理、§505熵值计算公理、§506成本透视公理

#### 3.4.1 组件分布
- **Gateway监控族**: ConstitutionMonitor、EntropyService、CostTracker、UIStateSyncService、ControlPanelSyncService、AgentExecutionVisualizer、StreamingOptimizer、TaskQueueVisualizer、TaskSchedulerEnhanced、PerformanceMetricsEnhanced、CostPerspective
- **主运行时监控**: `server/services/authority/AuthorityMonitoringService.ts`
- **基础设施监控**: `server/monitoring/ResourceMonitor.ts`、`server/monitoring/PerformanceOptimizer.ts`、健康检查相关实现

#### 3.4.2 集成状态说明
- 监控代码分布于`server/gateway/monitoring/`、`server/services/authority/AuthorityMonitoringService.ts`、`server/monitoring/`
- 默认 npm 主运行态会挂载 authority 监控路由与开发态`/colyseus`面板（开发环境且未禁用时）
- Gateway 监控工厂 `createMonitoringServices()` 当前未在 npm 主链路统一自动装配
...
- **用途**: 持久化的重要数据
- **特性**: 数据安全，版本控制
- **实现**: Qdrant/文件系统（PostgreSQL仅作为可选外部规划）
- **数据**: 知识库文件、配置信息、历史记录、向量索引
...
### 8.2 HTTP REST API接口
**说明**: 以下端点描述的是`server/gateway/`独立 Gateway 能力边界，不代表 `npm run dev` 的默认主链路全部自动开放。
**OpenAI兼容端点**:
```typescript
// 聊天补全
...
### 12.1 核心实现文件
- **默认主运行时入口**: `server/index.ts`
- **Legacy入口**: `src/index.ts`
- **Gateway模块入口**: `server/gateway/index.ts`
- **Agent引擎**: `server/gateway/agent-engine.ts`
- **插件系统**: `server/plugins/` + `server/gateway/plugins/`
- **监控系统**: `server/services/authority/AuthorityMonitoringService.ts` + `server/gateway/monitoring/` + `server/monitoring/`
