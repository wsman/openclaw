...
describe("handleControlUiHttpRequest", () => {
  ...
  async function tryCreateSymlink(targetPath: string, linkPath: string): Promise<boolean> {
    try {
  ...
  it("rejects avatar symlink paths from resolver", async () => {
    ...
    try {
      ...
      if (!(await tryCreateSymlink(outsideFile, linkPath))) {
        return;
      }
    ...
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        try {
          ...
          await fs.mkdir(assetsDir, { recursive: true });
          await fs.writeFile(outsideFile, "outside-secret\n");
          if (!(await tryCreateSymlink(outsideFile, path.join(assetsDir, "leak.txt")))) {
            return;
          }
  ...
  it("allows symlinked assets that resolve inside control-ui root", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        if (!(await tryCreateSymlink(filePath, path.join(assetsDir, "linked.txt")))) {
          return;
        }
    ...
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        try {
          ...
          if (!(await tryCreateSymlink(outsideIndex, path.join(tmp, "index.html")))) {
            return;
          }
