...
function mapDecisionErrorCodeToGatewayErrorCode(
  errorCode: unknown,
...
export function resolveNegentropyPluginConfig(
  ...
  return {
    mode: DECISION_MODES.includes(config.mode as DecisionMode)
  ...
  return async (
    ...
    if (result.decision.action === "REWRITE") {
      ...
      return {
        ...
        traceId: result.request.traceId,
        details:
          result.decision.policyTags && Object.keys(result.decision.policyTags).length > 0
