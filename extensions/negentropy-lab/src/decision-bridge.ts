...
function deriveStatusUrl(serviceUrl: string): string {
  if (serviceUrl.endsWith("/decision")) {
    return `${serviceUrl.slice(0, -"/decision".length)}/status`;
  }
  if (serviceUrl.endsWith("/internal/openclaw")) {
    return `${serviceUrl}/status`;
  }
  return serviceUrl;
}
...
function normalizeErrorCode(value: unknown): string | undefined {
  ...
  if (
    Object.values(DECISION_ERROR_CODES).includes(
      normalized as (typeof DECISION_ERROR_CODES)[keyof typeof DECISION_ERROR_CODES],
    )
  ) {
    return normalized;
  }
...
function normalizeDecisionResponse(raw: unknown, request: DecisionRequest): DecisionResponse {
...
function toCanonicalRequest(request: DecisionRequest): CanonicalDecisionRequest {
  ...
  const canonical: CanonicalDecisionRequest = {
    ...
    ...(normalizeRecord(request.headers)
      ...
      : {}),
    ...(normalizeString(request.httpMethod)
      ? { httpMethod: normalizeString(request.httpMethod) }
      : {}),
    ...(explicitAuthMeta
      ? { authMeta: explicitAuthMeta }
      : authMapped.authMeta
        ? { authMeta: authMapped.authMeta }
        : {}),
    ...(scopes ? { scopes } : {}),
  };
...
export class DecisionBridge {
  ...
  getStatusUrl(): string {
    return deriveStatusUrl(this.config.serviceUrl);
  }
  ...
  async probeReachability(): Promise<ReachabilityProbe> {
    const checkedUrl = this.getStatusUrl();
    try {
    ...
    return {
      ...
      rewrittenMethod:
        decision.action === "REWRITE"
          ? (normalizeString(decision.method) ?? request.method)
          : undefined,
      rewrittenParams:
        decision.action === "REWRITE"
