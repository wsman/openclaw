...
describe("workflow client", () => {
  it("calls workflow endpoints with the expected paths", async () => {
    const fetchSpy = vi
      ...
      .mockResolvedValue({
        ok: true,
        text: async () => JSON.stringify({ run: { runId: "wf-1" }, actions: [] }),
      } as Response)
      ...
      .mockResolvedValueOnce({
        ok: true,
        text: async () => JSON.stringify({ run: { runId: "wf-2" }, actions: [] }),
      } as Response)
      .mockResolvedValueOnce({
        ok: true,
        text: async () => JSON.stringify({ run: { runId: "wf-1" }, actions: [] }),
      ...
      .mockResolvedValueOnce({
        ok: true,
        text: async () => JSON.stringify({ total: 1, runs: [] }),
      } as Response)
      .mockResolvedValueOnce({
        ok: true,
        text: async () => JSON.stringify({ runId: "wf-1" }),
      } as Response)
      .mockResolvedValueOnce({
        ok: true,
        text: async () => JSON.stringify({ runId: "wf-1", trace: [], total: 0 }),
      } as Response);
  ...
  it("reports workflow reachability through the list surface", async () => {
    const fetchSpy = vi.fn<typeof fetch>().mockResolvedValue({
      ok: true,
      text: async () => JSON.stringify({ total: 0, runs: [] }),
    } as Response);
