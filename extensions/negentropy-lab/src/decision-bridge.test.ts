...
describe("negentropy decision bridge", () => {
  ...
  it("probes the derived decision status endpoint for host-facing diagnostics", async () => {
    const fetchSpy = vi.fn<typeof fetch>().mockResolvedValue({
      ok: true,
      status: 200,
    } as Response);
    ...
    const probe = await bridge.probeReachability();
    ...
    expect(fetchSpy).toHaveBeenCalledWith(
      "http://127.0.0.1:3000/internal/openclaw/status",
