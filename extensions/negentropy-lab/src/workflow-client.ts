...
export type WorkflowReachabilityProbe = {
  reachable: boolean;
  checkedUrl: string;
  detail: string;
};
...
export class WorkflowClient {
  ...
  async probeReachability(): Promise<WorkflowReachabilityProbe> {
    const checkedUrl = `${this.config.baseUrl}?limit=1`;
    try {
      await this.listRuns({ limit: 1 });
      return {
        reachable: true,
        checkedUrl,
        detail: "HTTP 200",
